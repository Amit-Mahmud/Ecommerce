...
class adminBack{
    ...
    function unpublish_category($id){
        ...
        if(mysqli_query($this->conn, $query)){
            $msg = "Category Unpublished Successfully!";
            return $msg;
        }   
        }
    function delete_category($id){
        ...
        function getCatinfo_toupdate($id){
            ...
            function add_product($data){
                ...
                if($pdt_ext == 'jpg' or $pdt_ext == 'png' or $pdt_ext == 'jpeg'){
                    if($pdt_img_size <= 2097152){
                        $query= "INSERT INTO products(pdt_name, pdt_price, pdt_des, pdt_ctg, pdt_img, pdt_status) VALUE
                        ('$pdt_name', $pdt_price, '$pdt_des', $pdt_ctg, '$pdt_img_name', $pdt_status)";
                        if(mysqli_query($this->conn, $query)){
            ...
            function update_product($data){
                ...
                if($pdt_ext == 'jpg' or $pdt_ext == 'png' or $pdt_ext == 'jpeg'){
                    if($pdt_img_size <= 2097152){
                        $query= "UPDATE products SET pdt_name='$pdt_name', pdt_price=$pdt_price, pdt_des='$pdt_des', pdt_ctg=$pdt_ctg, pdt_img='$pdt_img_name', pdt_status=$pdt_status WHERE
                        pdt_id=$pdt_id";
                        if(mysqli_query($this->conn, $query)){
                            move_uploaded_file($pdt_tmp_name, 'upload/'. $pdt_img_name);  
            ...
            function user_register($data){
                ...
                if($row==1){
                    ...
                    if(strlen($user_mobile)<11 or strlen($user_mobile)>11){
                        $msg = "Your Mobile Number Should Not be less thn 11 Digits!";
                        return $msg;
                    }else{
                    $query= "INSERT INTO users(user_name, user_firstname, user_lastname, user_email, user_password, user_mobile, user_roles) VALUE
                        ('$username','$user_firstName','$user_lastName','$useremail','$user_pass',$user_mobile,$user_roles)";
                        if(mysqli_query($this->conn,$query)){
