<?php 
    ...
    if(isset($_GET['status'])){
        ...
        if($_GET['status']=='publish'){
            $msg = $obj_adminBack->publish_category($get_id);
        }elseif($_GET['status']=='unpublish'){
            $msg = $obj_adminBack->unpublish_category($get_id);
        ...
        <tr>
            ...
            <td><?php 
                if( $ctg['ctg_status']==0){
                    echo "Unpublished";
                    ?>
                <a class="btn btn-sm btn-success" href="?status=publish&&id=<?php echo $ctg['ctg_id']; ?>">Make
                    Published</a>
                <?php
                }else{
                    echo "Published";
                    ?>
                <a class="btn btn-sm btn-danger" href="?status=unpublish&&id=<?php echo $ctg['ctg_id']; ?>">Make
                    Unpublished</a>
                <?php
                } 
                ?>
            ...
            <td>
                <a href="edit-catagory.php?status=edit&&id=<?php echo $ctg['ctg_id']; ?>">Edit</a>
